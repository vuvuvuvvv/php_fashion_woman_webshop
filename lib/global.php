...
// Trả về tên của hệ thống thông tin, cửa hàng
// ví dụ: iShop, shop quần áo, cửa hàng hoa v.v...
function web_name()
{
...
function web_tel()
{
	return web_telephone();
}

// Trả về địa chỉ của công ty, cửa hàng
...
/**
 * Chỉnh kích thước ảnh và trả về đường dẫn url của ảnh mới sinh ra.
 * Nếu bức ảnh gốc không tồn tại trên ổ cứng thì dùng ảnh rỗng '/images/placeholder.png' để thay thế.
 * @update 2021.04.24 21h26
 */
function url_image_resized($relative_path, $width, $height)
{
    if (is_file(DIR_IMAGE . $relative_path))
    {
        $image = img_resize($relative_path, $width, $height);
    }
    else
    {
...
/**
 * Làm ngắn nội dung ban đầu.
 * @param string $text
 * @param number $length
 * @return string
 *
 * @since 2021.04.24 10h14
 * @version 2021.04.24 10h14
 */
function text_short($text, $length=100)
{
    return utf8_substr(strip_tags(html_entity_decode($text, ENT_QUOTES, 'UTF-8')), 0, $length) . '..';
}

/**
 * @abstract mã html của bản đồ Google Map
 * @param number $length
 * @return string
 *
 * @since 2021.04.24 10h14
 * @version 2021.04.24 10h14
 */
function html_google_map()
{
    global $settings;
    return $settings['html_google_map_embed'];
}
 ...
 * @return string
 * 
 * @abstract Định dạng giá tiền theo kiểu Việt Nam đồng, dấu phảy phân cách phần nghìn.
 * @since 2021.04.28 9h32
 * @version 2021.04.28 9h32
 */
function price_text_vnd($price)
{
