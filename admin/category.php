...
// Giới hạn phân trang mặc định = 5, nhưng nếu database có yêu cầu khác thì chuyển sang
...
// Thực hiện truy vấn
$category_total = categoryGetTotal($filter_data);
...
//  Ví dụ: sắp xếp theo name, sort_order
// Phải có bí danh tên bảng phía trước tên cột đề phòng câu sql trong thư viện hàm
// sử dụng phép nối bảng.
$sort_name        = "/admin/category.php?sort=cd.name".$url;
$sort_category_id = "/admin/category.php?sort=cd.category_id".$url;
$sort_status      = "/admin/category.php?sort=cd.status".$url;
$sort_featured    = "/admin/category.php?sort=cd.featured".$url;
$sort_top         = "/admin/category.php?sort=cd.top".$url; 
$sort_sort_order  = "/admin/category.php?sort=cd.sort_order".$url;
$sort_date_added  = "/admin/category.php?sort=cd.date_added".$url; 
...
paginate($category_total, $page,$limit, "/admin/category.php".$url);
